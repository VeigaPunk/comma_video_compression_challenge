...
"""Inflate the rhnerv_latent_polish archive.

Member layout (single ZIP member `x`, ZIP_STORED):
  [ctx container: 7-byte header | decoder section | latent section |
   selector section]

This is the PR #112 (`rhnerv_comma`) payload format WITHOUT the trailing
607-byte latent sidecar: the sidecar's corrections plus ~1,565 further
exact-score-verified one-step latent code adjustments are baked directly
into the stored latent codes. Decoder weight bytes are byte-identical to
PR #101 / #95; the FEC6 selector is byte-identical to PR #110. The decode
chain below is the EXACT #110/#112 chain: HNeRVDecoder, 16-pair batches,
bicubic 874x1164 upsample (align_corners=False), the #98 channel biases
(frame0 R-1, frame0 B-1, frame1 G-1), clamp(0,255).round(), per-pair FEC6
selector transforms, uint8 NHWC streamed write. Device pinned to CPU.
...
def unpack_fec6_selector(selector_payload: bytes):
...
def apply_dynamic_mode(frame_chw: torch.Tensor, spec) -> torch.Tensor:
...
def apply_compact_selector_to_frames(frames_bchw, selector_codes, selector_specs, *, pair_start):
...
def parse_member(member_bytes: bytes):
    """ctx container (NO trailing sidecar) -> (state_dict, latents, codes, specs)."""
...
def inflate(src_bin: str, dst_raw: str) -> int:
    ...
    device = torch.device("cpu")  # pinned: CPU is the leaderboard axis
    ...
    with torch.inference_mode(), open(dst_raw, "wb") as fout:
        for i in range(0, N_PAIRS, 16):
            ...
            rounded = apply_compact_selector_to_frames(
                rounded, selector_codes, selector_specs, pair_start=i)
